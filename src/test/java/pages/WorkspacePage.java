...
public class WorkspacePage extends BasePage {
    ...
    private static final String SELECT_ACCESS_LVL = "//section//div[contains(text(),'%s')]";
    ...
    public void selectAccessLvl(accessLvl lvl) {
        switch (lvl) {
            case PUBLIC: {
                ...
                WebElement element = Waiter.waitElementToBeClickable(driver, By.xpath(String.format(SELECT_ACCESS_LVL, "Публичная")));
                element.click();
                break;
            }
            case PRIVATE: {
                log.info("Access level: " + PRIVATE);
                WebElement element = Waiter.waitElementToBeClickable(driver, By.xpath(String.format(SELECT_ACCESS_LVL, "Приватная")));
                element.click();
                break;
            }
            case WORKING: {
                log.info("Access level: " + WORKING);
                WebElement element = Waiter.waitElementToBeClickable(driver, By.xpath(String.format(SELECT_ACCESS_LVL, "Для рабочего")));
                element.click();
                break;
            }
        }
